...
match_hits.py is the python script I wrote. It outputs a tsv file containing all reciprocal BLAST hits and a tsv file containing the draft reconstruction reactions (GPRs not updated from iJO1366). Reactions from iJO1366 were only included if homologs in the cab strain satisfy the GPR. Additionally, the number of reactions included for each draft reconstruction is printed. 
